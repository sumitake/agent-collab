...
3. Capture artifact author-model provenance. If primary or artifact family is
   ...
   Send `primary` as `{}` so the host is observed. Never invent a
   ...
   route, and on a detected Claude host identity is observation-only, so no
   explicit identity field is accepted at all. If a route reports incomplete
   identity, let it fail closed rather than assembling an identity to satisfy
   it.
