...
class WireDescriptorSnapshot:
    ...
    zero_inference_readiness: Mapping[str, Any]
...
def validate_wire_descriptor(
    ...
    return WireDescriptorSnapshot(
        ...
        zero_inference_readiness=descriptor["zero_inference_readiness"],
...
def runtime_contract_snapshot() -> tuple[WireDescriptorSnapshot | None, str, str]:
    ...
    try:
        ...
        return None, digest, "runtime manifest is unreadable"
    _entry, wire, error = _manifest_entry(data)
    return wire, digest, error
...
def resolve_runtime() -> RuntimeResolution:
    ...
    entry, wire, error = _manifest_entry(data)
    if entry is None:
        return RuntimeResolution(RuntimeStatus.MANIFEST_INVALID, manifest_digest=manifest_digest, wire=wire, error=error)
    if not entry:
        return RuntimeResolution(RuntimeStatus.UNAVAILABLE, manifest_digest=manifest_digest, wire=wire, error=error)
...
def invoke(*, envelope: object) -> RuntimeResult:
    ...
    try:
        response = runtime_bundle.load_closed_json_object(stdout)
        ...
        schema = resolution.wire.success_response if status is RuntimeStatus.OK else resolution.wire.failure_response
