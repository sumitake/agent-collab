...
{{ debate_defaults_block }}
description: Stage a structured multi-round adversarial debate between {{ primary_agent }} and {{ verifier_agent }} on a binary or near-binary proposition. Each side is assigned (or self-selects) an opposing position and defends it through openings, rebuttals, and an optional closing round; the agent then steps out of advocacy and synthesizes a verdict for the user. Use when the user says "debate this with {{ verifier_agent }}," "argue both sides," "steelman both positions," "play devil's advocate with {{ verifier_agent }}," "{{ primary_agent }} vs {{ verifier_agent }} on X," "have {{ verifier_agent }} argue the other side," or "make the case against." Also offer this proactively when the user is leaning hard one way on a high-stakes binary choice and a structured opposing case would stress-test the conviction better than a polite second opinion would.
---
...
- **The user explicitly asks for it** — "debate this with {{ verifier_agent }}," "argue both sides," "steelman both positions," "play devil's advocate with {{ verifier_agent }}," "{{ primary_agent }} vs {{ verifier_agent }} on X," "have {{ verifier_agent }} argue the other side," "make the case against," "what's the strongest counter-position."
...
**{{ primary_agent }}'s rebuttal:** Read {{ verifier_agent }}'s opening. Pick its two strongest arguments and rebut them specifically — not "but on the other hand," but "here is why that argument fails, is incomplete, or rests on a false assumption." Quote the argument before rebutting it so the reader can follow the chain.

**{{ verifier_agent }}'s rebuttal:** Send {{ primary_agent }}'s opening through
the same sealed managed route (`{{ mcp_tool_ask }}`;
{{ debate_call_params_short }}):

```
Continuing the debate. The opposing side ([PRO|CON]) just argued:

[paste {{ primary_agent }}'s opening verbatim]
...
**PRO rebuttal ({{ primary_agent }}):**
REBUTTAL 1 (target: ...): ...
REBUTTAL 2 (target: ...): ...

**CON rebuttal ({{ verifier_agent }}):**
REBUTTAL 1 (target: ...): ...
REBUTTAL 2 (target: ...): ...
...
### Synthesis ({{ primary_agent }}, stepping out of advocacy)
...
When relaying to the user, **quote {{ verifier_agent }} directly** for the sharp objections in its openings and rebuttals — paraphrasing softens them, and softened objections are exactly what this skill exists to prevent.
...
- **Conceding mid-debate.** Both sides defend until synthesis. {{ verifier_agent }} conceding in Round 2 ("yes, the PRO side has a fair point...") defeats the purpose. If {{ verifier_agent }}'s rebuttal reads as conciliatory, push back: "this is a debate, defend the side you were assigned forcefully."
