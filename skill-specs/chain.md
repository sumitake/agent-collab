...
`{{ package_name }}:<skill>@<version>`. Reject unsupported fields rather than
...
- **{{ primary_agent }} is about to manually invoke 3+ skills in sequence** on the same artifact, where the same sequence is likely to repeat. Codify it as a chain up-front; future runs get reproducibility + audit trail by construction.
...
- **Dispatch based on `runner:`**:
  ...
  - `runner: <primary>` (the lowercased primary-agent identifier as declared in the chain spec): process the rendered `instruction:` directly via {{ primary_agent }}'s own reasoning. Output formatted per the step's declared schema.
...
example, `{{ package_name }}:second-opinion@4.1.0`). Retired namespaces and
...
{{ primary_agent }} is the runtime. This has trade-offs vs an external code-native orchestrator:

- **Verifiability is weaker than a code runtime**: execution depends on {{ primary_agent }} consistently following these instructions, not on deterministic code paths. For strict-precision long-term needs, an external runtime is the right answer.
...
- **Versioning enforcement is honor-system**: `skill_hash` is computed for the audit, but resolving `@version` references depends on {{ primary_agent }} correctly parsing and looking up the version. Halt-on-not-found is the safety mechanism.
