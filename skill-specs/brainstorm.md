...
- **The user explicitly asks for it** — "brainstorm with {{ verifier_agent }}," "let's ideate," "what are some options," "think this through with {{ verifier_agent }}," "thinking partner," "give me alternatives," "spitball this with {{ verifier_agent }}."
...
- The question has a single correct answer that a quick lookup would resolve. Call `{{ mcp_tool_ask_short }}` directly for fact-finding.
...
- `existingContext` — what {{ primary_agent }} or the user has already tried, decided, or ruled out (so the brainstorm does not waste turns retreading).
...
- For follow-up clarification or fact-checking of a specific candidate, call `{{ mcp_tool_ask }}` directly with the focused question. Reserve `{{ mcp_tool_brainstorm }}` for the divergent passes.
...
|---|---|---|
| Product management | Naming a new product line | Candidates spanning the literal-descriptive ↔ abstract-evocative axis, including names {{ primary_agent }}'s training would have under-weighted |
...
- **Brainstorming a question that has a single right answer.** "What is the capital of France" does not need a divergent thinking partner. Route fact-finding through `{{ mcp_tool_ask_short }}` directly.
