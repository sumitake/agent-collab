...
- **The user explicitly asks for it** — "audit this calculation," "double-check my math with {{ verifier_agent }}," "verify these computations," "check this trace," "is this cap table right," "re-derive this with {{ verifier_agent }}," "audit my arithmetic," "logic-check this," "is this number right."
- **{{ primary_agent }} has just done long arithmetic or a financial calculation** with intermediate quantities — cap tables, tax math, multi-currency aggregates, unit conversions, percentage-of-percentage chains, depreciation schedules.
- **{{ primary_agent }} has just done an algorithmic trace** — DP table fill, graph traversal, constraint propagation, BFS/DFS over a structured input — where a wrong intermediate state silently ruins the answer.
...
### 2. Ask {{ verifier_agent }} to re-derive — not to review

This is the load-bearing methodological discipline of the skill. **Send the problem and the constraints; do NOT send {{ primary_agent }}'s derivation or answer.** Asking the verifier to "review the math" reliably anchors on the existing trace; independent re-derivation does not.
...
- **Switch to transition critique**: send {{ primary_agent }}'s trace and ask the verifier to verify each transition's correctness, not re-derive the whole thing.
...
- **Withholding the answer from the user while waiting on the audit.** Show {{ primary_agent }}'s work; reconcile in public if needed. Latency-without-purpose is wasted user time.
- **Sending {{ primary_agent }}'s reasoning and asking the verifier to "audit my steps."** This is the failure mode the skill exists to prevent. The verifier anchors on the trace; the audit's value evaporates. Re-derivation requires the problem statement only.
...
- **Trying to reconcile when the verifier's derivation is itself incoherent.** Evaluate the verifier's work quality first. If it is garbled or internally inconsistent, do not merge it — flag it and re-check {{ primary_agent }}'s math against constraints manually.
