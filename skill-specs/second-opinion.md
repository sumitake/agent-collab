...
- **The user explicitly asks for it** — "second opinion," "what does {{ verifier_agent }} think," "sanity-check this," "cross-check," "have {{ verifier_agent }} review."
...
Do not dump the artifact at {{ verifier_agent }} with a vague "thoughts?" — that produces flattery-shaped paragraphs. Ask for four specific things:
...
The `pro` tier resolves on this side to {{ tier_pro_resolves_to }}, which is the tier configured for slow, skeptical analysis — exactly what a cross-check wants. The faster `flash` tier is the wrong choice here; it optimizes for throughput, not for finding objections. Ensure each panelist receives the **whole** artifact — a divergence that is actually an artifact of one model truncating the context is a false signal, not a real disagreement.
...
>
> **Panel confidence:** <each panelist's H/M/L>. **Where the panel agrees with {{ primary_agent }}:** <briefly, last>.
...
- **Using this for trivia or quick lookups.** Call `{{ mcp_tool_ask_short }}` directly. The framing overhead and `pro`-tier latency are not worth it for "what does X mean."
- **Treating {{ verifier_agent }} as an oracle.** A second opinion is one more input, not adjudication. The synthesis step is non-optional — the agent owes the user a call, not a relay.
